...
path = '/home/martyna/Documents/EBioMedicine_revision/ICGC_data/output'

df_all_mut = pd.read_csv(
    path + '/all_mutations_loc.csv'
)
...
df_coordinates = pd.read_csv(
    '/home/martyna/Documents/EBioMedicine_revision/ICGC_data/files_for_mirnaome_mirbase22.1/coordinates_withseq.bed',
    sep='\t',
    header=None
)
...
df_all_mut.to_csv(
    path + '/all_mutations_weights_new.csv'
)

df_localization = pd.read_csv(
    '/home/martyna/Documents/EBioMedicine_revision/ICGC_data/files_for_mirnaome_mirbase22.1/localizations.csv'
)
...
df_localization.to_csv(
    '/home/martyna/Documents/EBioMedicine_revision/ICGC_data/files_for_mirnaome_mirbase22.1/localizations_weight_new.csv'
)
...
coords = pd.read_csv(
            '/home/martyna/Documents/EBioMedicine_revision/ICGC_data/files_for_mirnaome_mirbase22.1/coordinates.bed',
            sep='\t', header=None)
...
if df_temp_raw.shape[0] > 0:
    ...
    # 19 is number of cohorts (18) + PAN-Cancer
    df_temp['FDR'] = df_temp.apply(lambda x: (x['order'] / (1914 * 19)) * 0.01, axis=1)
    df_temp['signif'] = df_temp['binom'] < df_temp['FDR']

    max_order = df_temp[df_temp['signif']]['order'].max()

    df_temp['BH'] = df_temp.apply(lambda x: (x['binom'] * 1914 * 19) / x['order'], axis=1)
    ...
    df_temp2['FDR'] = df_temp2.apply(lambda x: (x['order'] / (1914 * 19)) * 0.01, axis=1)
    df_temp2['signif'] = df_temp2['binom'] < df_temp2['FDR']
    max_order2 = df_temp2.loc[df_temp2['signif'], 'order'].max()
    df_temp2['BH'] = df_temp2.apply(lambda x: (x['binom'] * 1914 * 19) / x['order'], axis=1)
...
df_occur.to_excel(path + '/weight_summary_new.xlsx', index=False)
df_mutations_weight_summary.to_excel(
    path + '/weight_summary_mutations_new.xlsx', index=False)
df_mutations_summary.to_excel(
    path + '/gene_summary_mutations_new.xlsx', index=False)
